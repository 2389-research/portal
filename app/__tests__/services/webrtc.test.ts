...
describe('WebRTCManager', () => {
  ...
  describe('Connection Lifecycle', () => {
    ...
    test('should close the peer connection and clean up resources', async () => {
      ...
      const mockRemoteStream = createMockMediaStream('remote-stream-id', 'remote-audio', 'remote-video');
  ...
  describe('Media Stream Integration', () => {
    ...
    test('should get remote streams', async () => {
      ...
      const mockRemoteStream1 = createMockMediaStream('remote-stream-1', 'remote-audio-1', 'remote-video-1');
      const mockRemoteStream2 = createMockMediaStream('remote-stream-2', 'remote-audio-2', 'remote-video-2');
  ...
  describe('Negotiation', () => {
    ...
    test('should create offer successfully', async () => {
      ...
      expect(mockPeerConnection.setLocalDescription).toHaveBeenCalledWith({ type: 'offer', sdp: 'mock-sdp' });
    ...
    test('should process offer successfully', async () => {
      ...
      expect(mockPeerConnection.setLocalDescription).toHaveBeenCalledWith({ type: 'answer', sdp: 'mock-sdp' });
    ...
    test('should throw when processing offer with uninitialized connection', async () => {
      ...
      const offer = { type: 'offer', sdp: 'remote-sdp' };
      await expect(webrtcManager.processOffer(offer)).rejects.toThrow('Peer connection not initialized');
    });
    ...
    test('should throw when processing answer with uninitialized connection', async () => {
      ...
      const answer = { type: 'answer', sdp: 'remote-sdp' };
      await expect(webrtcManager.processAnswer(answer)).rejects.toThrow('Peer connection not initialized');
    });
  });
  ...
  describe('Error Handling', () => {
    ...
    test('should handle error when addIceCandidate fails', async () => {
      ...
      // Try to add ICE candidate
      const mockIceCandidate = { candidate: 'mock-candidate' } as unknown as RTCIceCandidate;
      await expect(webrtcManager.addIceCandidate(mockIceCandidate)).rejects.toThrow('addIceCandidate failed');
    });
  });
});